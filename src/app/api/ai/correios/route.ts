...
const TMP_CACHE_DIR = path.join(os.tmpdir(), 'ai-cache', 'correios');
...
function buildPrompt(payload: CorreiosAiPayload): string {
  ...
  return `
Você é um analista de dados do Ministério Público da Paraíba (MPPB) analisando despesas com serviços de Correios.
...
Custo médio por objeto: R$ ${kpis.custoMedio.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
